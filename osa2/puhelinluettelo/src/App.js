  ...
  const addPerson = (event) => {
    ...
    if (!persons.some(p => p.name === personObject.name)){
      NumberService
        ...
        .catch(error => {
          setNotification(`Adding '${personObject.name}' failed! error: ${error.response.data.body}`, 'error')
        })
    } else {
      ...
      if (window.confirm(msg)){
            NumberService
              .replace(oldPerson, personObject)
                ...
                .catch(error => {
                  setNotification(`Changing ${oldPerson.name}'s number failed! error: ${error.response.data.body}`, 'error')
                })
          }
    }
    setNewName('')
